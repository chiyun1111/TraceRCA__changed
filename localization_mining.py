...
def inject_noise(df, ratio=0):
    """ we investigated its influence by randomly reversing
    the anomaly detection results with probability noise ratio,
    which simulates ineffective anomaly detection

    :param df: the data with the anomaly invocations detection
    :param ratio: the probability noise ratio
    :return: the data with randomly reversing the anomaly detection results with probability noise ratio
    """
    rng = np.random.default_rng(2021)
    df = df.copy()
    df = df.drop(columns=['source', 'target']).reset_index()
    # Inject a certain probability of noise into  anomaly invocations detection
    for idx in range(len(df)):
        if rng.random() < ratio:
            df.loc[idx, PREDICT_COLUMN] = 1 - df.iloc[idx][PREDICT_COLUMN]
    abnormal_traces = set(df.loc[df.predict == 1, 'trace_id'].unique())
    # Inject a certain probability of noise(0.01) into anomaly trace detection
    if ratio == 0:
        for idx in range(len(df)):
            if df.iloc[idx]['trace_id'] in abnormal_traces and rng.random() < 0.01: #添加1%的trace检测噪声
...
def main(input_file, output_file, min_support_rate, quiet, k, enable_prfl):
    """ Suspicious Root-Cause Microservice Set Mining and Microservice Ranking

    :param input_file: the data with the anomaly invocations detection
    :param output_file: the root-cause microservice list by the sorted suspicious score
    :param min_support_rate: the support threshold of a microservice set in abnormal traces
    :param quiet: whether to print the result to the terminal
    :param k: the top-k suspicious microservice sets.
    :param enable_prfl: not used
    :return: the root-cause microservice list by the sorted suspicious score
    """
    ...
    for noise in [0]:
        injected_file = inject_noise(input_file, noise)  #注入噪声
        tic = time.time()
        fp = tracerca(
            injected_file,
...
class ItemsetHandler:
    """Get relevant data metrics for each set of microservices
    """
    def __init__(self, data: pd.DataFrame, **kwargs):
        trace_ids, itemsets = self.gen_itemsets(data, **kwargs)
        self.trace_ids = trace_ids  #trace
        self.itemsets = itemsets    #每个trace中包含的微服务集
        self.items = reduce(lambda a, b: a | b, map(lambda x: x[0], self.itemsets)) \
                     - kwargs.get("forbidden_names", frozenset()) #获取所有的微服务
        ...
        for trace_id, (node_set, label) in zip(trace_ids, self.itemsets):
            for _item in node_set:
                item2trace_id_sets[_item].add(trace_id)  #包含该微服务的trace集合
            if label:
                for _item in node_set:
                    item2abnormal_trace_id_sets[_item].add(trace_id) #包含该微服务的异常trace集合

        self._traces_containing_item = {key: frozenset(value) for key, value in item2trace_id_sets.items()} #包含该微服务的trace集合
        self._abnormal_traces_containing_item = {
            key: frozenset(value) for key, value in item2abnormal_trace_id_sets.items()
        }  #包含该微服务的异常trace集合

        self.p_b = len(list(filter(lambda x: bool(x[1]), self.itemsets))) / len(self.itemsets) #trace异常的比例

        self.p_a_given_b, self.frequent_patterns = self._frequent_pattern_on_abnormal(**kwargs) #the support and frequent pattern
        ...
        for itemset, abnormal in self.itemsets:
            if abnormal:
                self.abnormal_itemsets_counts[itemset] += 1  #每个微服务集的异常trace异常数量
        self.abnormal_itemset_total_count = sum(self.abnormal_itemsets_counts.values()) #所有微服务集合集trace异常总数
    ...
    def _frequent_pattern_on_abnormal(self, **kwargs):
        """Get frequent pattern that satisfy support and its support

        """
        min_support_rate = kwargs.get("min_support_rate", 0.1)
        abnormal_itemsets = list(map(lambda x: x[0], filter(lambda x: bool(x[1]), self.itemsets))) #异常的trace集合
        te = TransactionEncoder()
        te_ary = te.fit_transform(abnormal_itemsets)
        df = pd.DataFrame(te_ary, columns=te.columns_)
        ret = fpgrowth(df, min_support=min_support_rate, use_colnames=True) #获取频繁项集
    ...
    def _get_confidence(self):
        """
        to get the confidence of a microservice set (denoted as P(Y |X)),
        which represents the percentage of abnormal traces among all
        traces passing through all microservices in the set
        """
        confidence = {}
    ...
    def gen_itemsets(
        ...
        """To get the microservice set contained in each trace and
        anomaly detection results for each trace
        """
        ...
        trace_predict_ans = trace_group[predict_column].max() #选取每个trace的predict的最大值
        node_sets = list(
...
class TraceRCA:
    ...
    def __call__(self, input_file: pd.DataFrame, enable_PRFL=False, **kwargs) -> List[str]:
        ...
        def in_out_diff(_item, _pattern, return_="diff") -> float:
            """
            It is calculated by the difference between the numbers of
            traces that contain incoming/outcoming abnormal invocations
            on the microservice among all those traces containing the suspicious set.
            """
            if len(_pattern) <= 0:
                return 0.
...
if __name__ == "__main__":
    input_file = r'E:\AIOPs\TraceRCA-main\A\uninjection\invo_anomaly_detection_2.pkl'
    output_file = r'E:\AIOPs\TraceRCA-main\A\uninjection\final_result_2.pkl'
    ...
    main(input_file = input_file,output_file = output_file,min_support_rate = min_support_rate,
         enable_prfl = enable_prfl,k = k,quiet = quiet)
